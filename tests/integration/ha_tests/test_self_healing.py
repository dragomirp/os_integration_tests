...
async def test_freeze_db_process(
    ...
    async with ops_test.fast_forward():
        ...
        try:
            ...
            for attempt in Retrying(stop=stop_after_delay(60 * 3), wait=wait_fixed(3)):
                with attempt:
                    new_primary_name = await get_primary(
                        ops_test, app, down_unit=primary_name
                    )
                    assert new_primary_name != primary_name
        finally:
...
async def test_full_cluster_restart(
    ...
    await change_patroni_setting(
        ops_test, "ttl", 600, patroni_password, use_random_unit=True
    )
    await change_patroni_setting(
    ...
    try:
        ...
        if process == PATRONI_PROCESS:
            ...
            for unit in ops_test.model.applications[app].units:
                awaits.append(
                    update_restart_condition(ops_test, unit, ORIGINAL_RESTART_CONDITION)
                )
    ...
    assert set(member_ips) == set(
        ip_addresses
    ), "not all units are part of the same cluster."
...
async def test_forceful_restart_without_data_and_transaction_logs(
    ...
    await run_command_on_unit(
        ops_test, primary_name, "snap stop charmed-postgresql.patroni"
    )
...
async def test_network_cut(ops_test: OpsTest, continuous_writes, primary_start_timeout):
    ...
    for unit_name in set(all_units_names) - {primary_name}:
        logger.info(
            f"checking for no connectivity between {primary_name} and {unit_name}"
        )
    ...
    logger.info(
        f"checking for no connectivity between {primary_name} and the controller"
    )
